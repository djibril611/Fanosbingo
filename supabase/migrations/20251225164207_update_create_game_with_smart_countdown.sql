/*
  # Smart Countdown Extension for Winner Card Viewing

  1. Changes
    - Update `create_game_with_server_time()` to check for recently finished games
    - Extend countdown proportionally if game finished within last 12 seconds
    - This ensures players viewing winner cards get fair countdown time

  2. Logic
    - If last game finished < 12 seconds ago: extend countdown to compensate
    - Formula: effective_countdown = base_countdown + (12 - time_since_finish)
    - Maximum countdown capped at 37 seconds (25 + 12)
    - Minimum countdown remains 25 seconds

  3. Purpose
    - Synchronize winner card display time with next game countdown
    - All players return from winner card to fair countdown in lobby
    - Prevent disadvantage for players who view full winner display
*/

-- Update create_game_with_server_time to extend countdown for recently finished games
